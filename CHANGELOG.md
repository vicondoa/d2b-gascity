...
- Start the city tmux server from `d2b-gascity.service` with `exit-empty off`
  and `TMUX_TMPDIR` under `/run/d2b-gascity`, without a dummy session the
  supervisor would adopt.
